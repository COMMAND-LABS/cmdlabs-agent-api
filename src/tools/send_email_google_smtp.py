"""
Send Plain-Text Email Tool via Gmail SMTP (App Password) — tool type: sendTxtEmailWithGoogleSmtp.
...
async def create_send_email_google_smtp_tool(
    ...
    **kwargs,
    ...
    Required tool_config keys (type = sendTxtEmailWithGoogleSmtp):
    ...
    if not credential_id:
        raise CredentialError(
            "Missing required field 'credentialId' in sendTxtEmailWithGoogleSmtp tool configuration"
        )
    ...
    print(
        f"[SEND EMAIL GOOGLE SMTP TOOL] HITL tool 'send_txt_email_with_google_smtp' ready — "
    ...
    async def send_email_impl(to_email: str, subject: str, body: str) -> str:
        """Queue an email for human approval before sending via Gmail SMTP."""
        print(f"\n{'='*60}")
        print(f"[SEND EMAIL GOOGLE SMTP TOOL] 📬 HITL: queuing email for approval")
        print(f"[SEND EMAIL GOOGLE SMTP TOOL]   To      : {to_email}")
        print(f"[SEND EMAIL GOOGLE SMTP TOOL]   Subject : {subject}")
        print(f"[SEND EMAIL GOOGLE SMTP TOOL]   Body    : {len(body)} chars")
        print(f"{'='*60}\n")
        ...
        try:
            ...
            print(f"[SEND EMAIL GOOGLE SMTP TOOL] ✅ PendingToolApproval created — id={approval_id}")
        except Exception as e:
            approval_db.rollback()
            print(f"[SEND EMAIL GOOGLE SMTP TOOL] ❌ Failed to create approval record: {e}")
            import traceback
            traceback.print_exc()
            return json.dumps({
                "success": False,
    ...
    return StructuredTool(
        ...
        name="send_txt_email_with_google_smtp",
